
<div >

        <div class="bg-dark text-center p-1">
          <h3> Registrar materia</h3>
        </div>

        <div class="text-center bg-info p-1 mt-1">
          <div class="h-100 w-50 d-inline-block">
                <form action="{{url('materia')}}" method="POST">
                <div class="form-group row">
                    <label for="codigo" class="col-sm-2 col-form-label">Codigo</label>
                    <div class="col-sm-10">
                    <input type="number" class="form-control" id="codigo" placeholder="Ingrese codigo de materia" name="codigoM">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    </div>
                </div>
                
                <div class="form-group row">
                    <label for="nombre" class="col-sm-2 col-form-label">Nombre</label>
                    <div class="col-sm-10">
                    <input type="text" class="form-control" id="nombre" placeholder="Ingrese nombre de materia" name="nombreM">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    </div>
                </div>

                
                <input type="submit" class="btn btn-primary" value="Registrar">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                </form>
          </div>
        </div>

        
        <div class=" mt-1">
          <table class="table">
            <thead class="thead-dark">
              <tr>
                <th scope="col">Codigo</th>
                <th scope="col">Nombre</th>
              </tr>
            </thead>
            <tbody class=" table-active text-dark">
              <tr>
                <th>2010010</th>
                <td>Introduccion a la programacion</td>
              </tr>
              <tr>
                <th>2010003</th>
                <td>Elementos</td>
              </tr>
            </tbody>
          </table>
          </div>
        
</div>
