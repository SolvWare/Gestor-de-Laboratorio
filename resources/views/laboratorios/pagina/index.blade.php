...
@section('navegacion')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb p-0 m-0 pl-5">
            ...
            <li class="breadcrumb-item active" href="/home">Lista de Laboratorios</li>
        </ol>
    </nav>
@endsection
                    ...
                    <table class="table table-sm table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                @endcan
                                @can('laboratoriodisables.destroy')
                        ...
                        <tbody>
                            @foreach ($laboratorios as $laboratorio)
                                <tr class="">
                                    <td class="pl-2">{{$laboratorio->nameL}}</td>
                                    <td>{{$laboratorio->capL}}</td>
                                    <td>{{$laboratorio->estado}}</td>
                                    @can('laboratorios.show')
                                        <td class="text-center">
                                            <form action="{{ route('laboratorios.show', $laboratorio->id) }}" method="GET">
                                                <button class=""><i class="fas fa-eye fa-lg"></i></button>
                                            </form>
                                        </td>
                                    @endcan
                                    @can('laboratorios.edit')
                                        <td class="text-center">
                                            <form action="{{ route('laboratorios.edit', $laboratorio->id) }}" method="GET">
                                                <button class=""><i class="fas fa-edit fa-lg"></i></button>
                                            </form>
                                        </td>
                                    @endcan
                                    @can('laboratorios.destroy')
                                        <td class="text-center">
                                    ...
                                    @can('laboratoriodisables.destroy')
                                        <th class="text-center">
                                            <form action="{{ route('laboratoriodisables.destroy', $laboratorio->id) }}" method="POST">
                                                {{ csrf_field() }}
                                                <input type="hidden" name="_method" value="DELETE">
